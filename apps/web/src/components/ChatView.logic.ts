...
export function shouldClearSubmittedThreadGoalDraft(input: {
  ...
  return (
    input.submittedPrompt === input.currentPrompt &&
    input.imageCount + input.fileCount === 0 &&
...
export function deriveComposerSendState(options: {
  ...
  const trimmedPrompt = trimComposerMessageText(stripInlineContextReferences(options.prompt));
  ...
  return {
    ...
    hasSendableContent:
      hasVisibleThreadGoalText(trimmedPrompt) ||
...
/** Applies the same whitespace policy used by /goal classification. */
export function trimComposerMessageText(prompt: string): string {
  return trimThreadGoalWhitespace(prompt);
}
