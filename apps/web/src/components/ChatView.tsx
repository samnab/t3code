...
export default function ChatView(props: ChatViewProps) {
  ...
  const confirmThreadExperiment = useCallback(async () => {
    ...
    if (
      ...
      shouldClearSubmittedThreadGoalDraft({
        ...
        imageCount: latestDraft.images.length,
        fileCount: latestDraft.files.length,
        contextCount:
  ...
  const timelineMessages = useMemo(() => {
    ...
    // Child results are represented by the Agents panel. Keep the
    // server-authored delivery in the provider conversation so the parent can
    // synthesize it, but do not duplicate that synthetic user turn here.
  ...
  const onSend = async (
    ...
    const promptWithDirectAnnotation = directAnnotation
      ? ensureInlineContextReferences(rawPrompt, [
    ...
    // Parse on the context-reference-stripped but untrimmed prompt: native
    ...
    const goalCommand =
      goalBootstrap === undefined
        ? parseThreadGoalCommand(stripInlineContextReferences(promptForSend))
        : null;
    if (goalCommand) {
    ...
    if (
      ...
      const followUp = resolvePlanFollowUpSubmission({
        // Keep policy whitespace intact while excluding inline context links
        // from the plan follow-up decision.
        draftText: stripInlineContextReferences(promptForSend),
    ...
    const messageTextForSend = trimComposerMessageText(
    ...
    let titleSeed =
      goalBootstrap?.goal ?? assistantCitationsToPlainText(stripInlineContextReferences(trimmed));
    if (!titleSeed) {
  ...
  const onSubmitPlanFollowUp = useCallback(
    async ({
      ...
      if (!hasVisibleThreadGoalText(trimmed)) {
        return false;
      }
          ...
          >
            ...
            <div className="relative flex min-h-0 flex-1 flex-col bg-background">
              ...
              <MessagesTimeline
                ...
                turnOutputUsage={paintOnlyDisplayedTimeline ? null : activeTurnOutputUsage}
