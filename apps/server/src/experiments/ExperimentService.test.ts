...
function makeRepo(
  ...
  return Effect.gen(function* () {
    ...
    yield* fileSystem.writeFileString(path.join(root, "score.txt"), "1\n");
    yield* fileSystem.writeFileString(path.join(root, "untouched.txt"), "owned by user\n");
    yield* Effect.promise(() => git(root, ["add", "score.txt", "untouched.txt"]));
...
it.layer(NodeServices.layer)("ExperimentService", (it) => {
    ...
    () =>
      Effect.gen(function* () {
        ...
        return yield* Effect.gen(function* () {
          ...
          assert.strictEqual(yield* fileSystem.readFileString(path.join(cwd, "score.txt")), "2\n");
          assert.strictEqual(
            yield* fileSystem.readFileString(path.join(cwd, "untouched.txt")),
            "owned by user\n",
          );
  ...
  it.effect("fails closed without restoring across an unrelated external edit", () =>
    Effect.gen(function* () {
      ...
      return yield* Effect.gen(function* () {
        ...
        yield* fileSystem.writeFileString(path.join(cwd, "untouched.txt"), "external edit\n");
  ...
  it.effect("restores an owned pending candidate during startup recovery", () =>
    Effect.gen(function* () {
      ...
      return yield* Effect.gen(function* () {
        ...
        assert.strictEqual(yield* fileSystem.readFileString(path.join(cwd, "score.txt")), "1\n");
        const summary = yield* service.get({ threadId: "thread-1" });
  ...
  it.effect("recovers a restore rename that happened before its path checkpoint", () =>
    Effect.gen(function* () {
      ...
      return yield* Effect.gen(function* () {
        ...
        const persisted = fixture.rows.get("thread-1");
        assert(persisted?.pending !== null && persisted?.pending !== undefined);
        yield* fileSystem.writeFileString(path.join(cwd, "score.txt"), "1\n");
        fixture.rows.set("thread-1", {
          ...persisted,
          phase: "restoring",
          armed: false,
  ...
  it.effect("restores the snapshot mode when recovery finds original content", () =>
    Effect.gen(function* () {
      ...
      return yield* Effect.gen(function* () {
        ...
        assert(persisted?.pending !== null && persisted?.pending !== undefined);
        yield* fileSystem.writeFileString(path.join(cwd, "score.txt"), "1\n");
        yield* fileSystem.chmod(path.join(cwd, "score.txt"), 0o755);
        fixture.rows.set("thread-1", {
          ...persisted,
          phase: "restoring",
          armed: false,
        ...
        yield* service.recoverAll();
        assert.strictEqual(
          (yield* fileSystem.stat(path.join(cwd, "score.txt"))).mode & 0o777,
          0o644,
        );
        assert.strictEqual((yield* service.get({ threadId: "thread-1" }))?.phase, "paused");
      }).pipe(Effect.provide(fixture.layer));
  ...
  it.effect("unstages and restores a candidate rejected by a normal commit hook", () =>
    Effect.gen(function* () {
      ...
      return yield* Effect.gen(function* () {
        ...
        assert.strictEqual(yield* fileSystem.readFileString(path.join(cwd, "score.txt")), "1\n");
        assert.strictEqual(
          (yield* Effect.promise(() => git(cwd, ["diff", "--cached", "--name-only"]))).stdout,
          "",
        );
      }).pipe(Effect.provide(fixture.layer));
    }),
  ...
  it.effect("restores instead of keeping when a commit hook exceeds the campaign deadline", () =>
    Effect.gen(function* () {
      ...
      return yield* Effect.gen(function* () {
        ...
        const before = (yield* Effect.promise(() => git(cwd, ["rev-parse", "HEAD"]))).stdout.trim();
        const identity = {
          threadId: "thread-1",
          providerInstanceId: "claude",
          providerSessionId: "experiment-thread-1-1",
        ...
        assert.strictEqual(yield* fileSystem.readFileString(path.join(cwd, "score.txt")), "1\n");
        assert.strictEqual(
          (yield* Effect.promise(() => git(cwd, ["rev-parse", "HEAD"]))).stdout.trim(),
          before,
        );
      }).pipe(Effect.provide(fixture.layer));
    }),
      ...
      return yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          yield* Effect.gen(function* () {
            ...
            assert.strictEqual(next.cwd, yield* fileSystem.realPath(cwd));
          }).pipe(
            Effect.provide(
              ExperimentLifecycleReactor.layer.pipe(
                Layer.provideMerge(fixture.layer),
      ...
      return yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          yield* Effect.gen(function* () {
            ...
            assert.strictEqual(releasedPreview.cwd, yield* fileSystem.realPath(cwd));
          }).pipe(
            Effect.provide(
              ExperimentLifecycleReactor.layer.pipe(
                Layer.provideMerge(fixture.layer),
  ...
  it.effect("restores a candidate when a check changes only its file mode", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const cwd = yield* makeRepo(10, 600, [
        [
          "node",
          "-e",
  ...
  it.effect("ignores a terminal profile whose old repository no longer exists", () =>
    Effect.gen(function* () {
      ...
      return yield* Effect.gen(function* () {
        ...
        assert.strictEqual(preview.cwd, yield* fileSystem.realPath(cwd));
