...
import { git } from "../experiments/Repository.ts";
import {
...
function makeRepo() {
  return Effect.gen(function* () {
    ...
    const root = yield* fileSystem.makeTempDirectoryScoped({ prefix: "t3-experiment-mcp-" });
    ...
    yield* Effect.promise(() => git(root, ["add", "score.txt"]));
...
it.layer(NodeServices.layer)("experiment MCP HTTP integration", (it) => {
      ...
      return yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          const toolsResponse = yield* httpClient.post("/mcp/experiment", {
            ...
            body: HttpBody.text(
              encodeJson({ jsonrpc: "2.0", id: 2, method: "tools/list", params: {} }),
              "application/json",
            ),
          });
          ...
          const approvedFile = path.join(cwd, "score.txt");
          const baselineBytes = yield* fileSystem.readFile(approvedFile);
          const baselineMode = (yield* fileSystem.stat(approvedFile)).mode;
          const baselineHead = (yield* Effect.promise(() =>
          ...
          expect(yield* fileSystem.readFile(approvedFile)).toEqual(baselineBytes);
          expect((yield* fileSystem.stat(approvedFile)).mode).toBe(baselineMode);
          expect((yield* Effect.promise(() => git(cwd, ["rev-parse", "HEAD"]))).stdout.trim()).toBe(
            baselineHead,
          );
          ...
          expect(malformed.error?.message).toContain(
            "Invalid parameters for tool 'experiment_apply'",
          );
          ...
          expect(yield* fileSystem.readFileString(path.join(cwd, "score.txt"))).toBe("2\n");
          ...
          const generalToolsResponse = yield* httpClient.post("/mcp", {
            ...
            body: HttpBody.text(
              encodeJson({ jsonrpc: "2.0", id: 9, method: "tools/list", params: {} }),
              "application/json",
            ),
          });
