...
describe("orchestration projector", () => {
  ...
  effectIt.effect("updates thread voice notifications from thread.voice-notifications-set", () =>
    Effect.gen(function* () {
      const createdAt = "2026-02-23T08:00:00.000Z";
      const updatedAt = "2026-02-23T08:00:05.000Z";
      const model = createEmptyReadModel(createdAt);
...
describe("thread goal projection", () => {
  ...
  effectIt.effect("sets, replaces, and clears a thread goal", () =>
    Effect.gen(function* () {
  ...
  effectIt.effect("preserves the goal when an unrelated meta update omits it", () =>
    Effect.gen(function* () {
      let model = createEmptyReadModel(now);
      const events = [
        THREAD_CREATED_INPUT,
  ...
  effectIt.effect("projects, updates, and clears the goal loop", () =>
    Effect.gen(function* () {
      const loop = {
        state: "running",
        mode: "t3",
