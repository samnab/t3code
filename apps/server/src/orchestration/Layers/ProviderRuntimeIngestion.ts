...
export function runtimeEventToActivities(
  ...
  switch (event.type) {
    ...
    case "account.rate-limits.updated": {
      const usageWindows = event.payload.limits.windows;
      ...
      return [
        {
          ...
          // The activity fold carries epoch millis; the pooled provider
          // snapshot keeps the ISO form. Merge is sparse, by window id.
          payload: {
            provider: event.provider,
            windows: usageWindows.map((window) => ({
              id: window.id,
              label: window.label,
...
const make = Effect.gen(function* () {
  ...
  const processRuntimeEvent = (inputEvent: ProviderRuntimeEvent) =>
    Effect.gen(function* () {
      if (
        inputEvent.type === "content.delta" &&
        inputEvent.payload.streamKind !== "assistant_text" &&
        inputEvent.payload.streamKind !== "reasoning_text" &&
        inputEvent.payload.streamKind !== "reasoning_summary_text"
      ) {
        return;
      }

      const thread = yield* resolveThreadRuntimeContext(inputEvent.threadId);
      if (!thread) return;
