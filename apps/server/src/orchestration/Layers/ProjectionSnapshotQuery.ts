...
const ProjectionProjectDbRowSchema = ProjectionProject.mapFields(
  Struct.assign({
    ...
    schedules: Schema.fromJsonString(Schema.Array(ProjectSchedule)),
  }),
);
...
const REQUIRED_SNAPSHOT_PROJECTORS = [
  ...
  ORCHESTRATION_PROJECTOR_NAMES.subagentRuns,
                  ...
                  row.deletedAt === null
                    ? Result.succeed({
                        ...
                        backgroundProcessCount:
                          threadBackgroundLiveness.getThreadBackgroundProcessCount(row.threadId),
            ...
            Effect.gen(function* () {
              ...
              const snapshot = {
                ...
                threads: threadRows.map((row): OrchestrationThreadShell => ({
                  ...
                  backgroundProcessCount: threadBackgroundLiveness.getThreadBackgroundProcessCount(
                    row.threadId,
                  ),
                  planProgress: threadPlanProgress.getThreadPlanProgress(row.threadId),
                })),
            ...
            : repositoryIdentityResolver.resolve(option.value.workspaceRoot).pipe(
                Effect.map((repositoryIdentity) =>
                  Option.some({
                    ...
                    schedules: option.value.schedules,
  ...
  const getThreadShellById: ProjectionSnapshotQueryShape["getThreadShellById"] = (threadId) =>
    Effect.gen(function* () {
      ...
      return Option.some({
        ...
        goalLoop: threadRow.value.goalLoop ?? null,
  ...
  const getThreadDetailByIdBounded = (
    ...
    Effect.gen(function* () {
      ...
      const [
        ...
        projectionSubagentRunRepository.listByThreadId({ threadId, limit: 100 }),
      ]);
      ...
      const thread = {
        ...
        messages: messageRows.map((row) => {
          ...
          if (row.origin !== null) {
            Object.assign(message, { origin: row.origin });
          }
          if (row.context !== null) {
