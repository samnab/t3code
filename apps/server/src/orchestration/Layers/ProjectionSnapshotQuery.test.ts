...
projectionSnapshotLayer("ProjectionSnapshotQuery", (it) => {
  it.effect("hydrates read model from projection tables and computes snapshot sequence", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(commandSnapshot.threads[0]?.pullRequests, expectedPullRequests);
      assert.deepEqual(
        commandSnapshot.threads[0]?.linkedPullRequest,
      ...
      const unlinkedSnapshot = yield* snapshotQuery.getSnapshot();
      assert.deepEqual(unlinkedSnapshot.threads[0]?.pullRequests, []);
      assert.equal("linkedPullRequest" in unlinkedSnapshot.threads[0]!, false);

      const unlinkedCommandReadModel = yield* snapshotQuery.getCommandReadModel();
      assert.deepEqual(unlinkedCommandReadModel.threads[0]?.pullRequests, []);
      assert.equal("linkedPullRequest" in unlinkedCommandReadModel.threads[0]!, false);
      ...
      const unlinkedDetail = yield* snapshotQuery.getThreadDetailById(ThreadId.make("thread-1"));
      assert.equal(unlinkedDetail._tag, "Some");
      if (unlinkedDetail._tag === "Some") {
        assert.deepEqual(unlinkedDetail.value.pullRequests, []);
        assert.equal("linkedPullRequest" in unlinkedDetail.value, false);
      }
