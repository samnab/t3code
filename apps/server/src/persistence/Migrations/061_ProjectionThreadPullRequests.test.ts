...
layer("061_ProjectionThreadPullRequests", (it) => {
...
it.layer(Layer.fresh(NodeSqliteClient.layerMemory()))("061 Azure legacy links", (it) => {
...
it.effect("upgrades the complete fork-at-60 schema to the fresh final schema", () =>
  Effect.gen(function* () {
    const linkedPullRequestJson = encodeJson({
      projectId: "project-fork",
      repository: "acme/widgets",
      number: 42,
      url: "https://github.com/acme/widgets/pull/42",
    });
    const branchPullRequestJson = encodeJson({
      repository: "acme/widgets",
      number: 43,
    ...
    const upgraded = yield* Effect.gen(function* () {
      ...
      yield* sql`
        INSERT INTO projection_threads (
          ...
          'thread-fork',
          'project-fork',
          'Fork at migration 60',
          '{"instanceId":"codex","model":"gpt-5.4"}',
          ${linkedPullRequestJson},
          ${branchPullRequestJson},
      ...
      const [thread] = yield* sql<{
        ...
        readonly branchPullRequestJson: string | null;
      }>`
        SELECT
          linked_pull_request_json AS "linkedPullRequestJson",
          branch_pull_request_json AS "branchPullRequestJson"
        FROM projection_threads
        WHERE thread_id = 'thread-fork'
      `;
      const links = yield* sql<PullRequestRow>`
        SELECT
          thread_id AS "threadId",
          host,
          repository,
        ...
        WHERE thread_id = 'thread-fork'
      `;
      const finalMigrations = yield* sql<{
    ...
    assert.deepStrictEqual(upgraded.thread, {
      linkedPullRequestJson,
      branchPullRequestJson,
    });
    assert.deepStrictEqual(upgraded.links, [
      {
        ...
        url: "https://github.com/acme/widgets/pull/42",
        source: "manual",
        linkedAt: "2026-09-12T00:00:01.000Z",
        snapshotJson: null,
        stackJson: null,
      },
    ]);
    assert.deepStrictEqual(upgraded.finalMigrations, [
      { migrationId: 60, name: "NativeChildDeliveryBatches" },
      { migrationId: 61, name: "ProjectionThreadPullRequests" },
      { migrationId: 62, name: "ProjectionThreadMessageContext" },
    ]);
