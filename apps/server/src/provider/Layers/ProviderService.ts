...
const makeProviderService = Effect.fn("makeProviderService")(function* (
   ...
   *
   * Browser access controls only the preview capability. Native delegation is
   * available independently; its service validates the live parent at use time.
   */
  /**
  ...
  const prepareMcpSession = (threadId: ThreadId, providerInstanceId: ProviderInstanceId) =>
    Effect.gen(function* () {
      // Delegation is always available; the preview capability is gated on the
      ...
      const credential = yield* issueMcpCredential({
        threadId,
        providerInstanceId,
        capabilities: browserEnabled ? ["preview", "delegation"] : ["delegation"],
      });
      if (credential) {
