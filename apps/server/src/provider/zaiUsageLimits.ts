...
let cache: { readonly at: number; readonly windows: ReadonlyArray<UsageLimitWindow> } | null = null;
...
async function fetchWindows(): Promise<ReadonlyArray<UsageLimitWindow>> {
  const now = Date.now();
...
export const zaiUsageLimitWindows: Effect.Effect<ReadonlyArray<UsageLimitWindow>> =
