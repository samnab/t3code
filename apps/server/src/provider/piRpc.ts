 ...
 * (`apps/server/src/orchestration-v2/Adapters/PiRpc.ts`, head
...
export interface PiRpcConnection {
   ...
   * Stop when the process may be wedged and `abort` cannot be trusted to
   * land. The transport fails and a fresh process respawns on the next
   * session start.
   */
