...
| Provider    | Install and authenticate                                                                     | Default binary |
| ----------- | -------------------------------------------------------------------------------------------- | -------------- |
| Codex       | Install [Codex CLI](https://developers.openai.com/codex/cli), then run `codex login`.        | `codex`        |
| Claude      | Install [Claude Code](https://claude.com/product/claude-code), then run `claude auth login`. | `claude`       |
| Cursor      | Install [Cursor CLI](https://cursor.com/cli), then run `agent login`.                        | `cursor-agent` |
| Grok Build  | Install [Grok Build CLI](https://x.ai/cli), then run `grok login`.                           | `grok`         |
| Pi          | Install [Pi](https://github.com/earendil-works/pi), then run `pi`, then `/login`.            | `pi`           |
| OpenCode    | Install [OpenCode](https://opencode.ai), then run `opencode auth login`.                     | `opencode`     |
| Antigravity | Install and sign in with Google from T3 Code's provider settings.                            | n/a            |
