...
Use the back arrow at the top of the detail view to return to the roster.

Sending a message is only available while the agent is live and its session still supports it.
When it isn't, the message box explains why: the agent already finished, its session is
read-only, or the provider doesn't support sending messages to subagents.
